...
describe Cadmium::Readability do
  ...
  it "should calculate correct values for all metrics when all test requirements are met" do
    long_subject.statistics.number_of_paragraphs.should eq(results["long"]["Number of paragraphs"])
    # long_subject.num_paragraphs.should eq(results["long"]["Number of paragraphs"])
    long_subject.statistics.number_of_sentences.should eq(results["long"]["Number of sentences"])
    # long_subject.statistics.number_of_words.should eq(results["long"]["Number of words"])
    ...
    long_subject.scores.linsear_write.should eq(results["long"]["Linsear Write Index"])
  end
  ...
  it "should calculate correct values for applicable tests for a short text or return 0.0 when the test requirements are not met" do
    short_subject.statistics.number_of_words.should eq(results["short"]["Number of words"])
